...
def load_rows():
    data_path = Path(__file__).resolve().parent / "data" / "repeated_windows.jsonl"
    rows = []
...
def publish_data():
    ...
    for row in rows:
        ...
        label = f"window={row.get('window')} step={row.get('step')}"
        print(f"publishing {label} -> {signal.state}")
